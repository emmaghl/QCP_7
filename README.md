...
```
The register is then called and both the bits and the basis are setup and printed to the terminal. They are then passed to the method `encode_message` which follows applies the flow diagram, seen in the report, to setup the qubits. The user knows this is compelted once the termianl prints:
```
